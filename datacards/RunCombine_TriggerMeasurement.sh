...
do
  ...
  do
    echo -e  "\n\n\n ----------------------------- ${iso} ${tauDecayMode} \n"
    # Combine datacards
    combineCards.py ../../output/${era}/datacard_tauPt_${iso}_WToTauNu_passingprobes${tauDecayMode}.txt ../../output/${era}/datacard_tauPt_${iso}_WToTauNu_failingprobes${tauDecayMode}.txt > datacard_tauPt_${iso}_WToTauNu${tauDecayMode}.txt
    # Run combine
